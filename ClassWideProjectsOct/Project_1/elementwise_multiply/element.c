 ...
 * gcc -O3 -march=native -ffast-math -funroll-loops -std=c11 element.c -o simd_element
 * gcc -O3 -march=native -ffast-math -funroll-loops -std=c11 -fopt-info-vec-optimized saxpy.c > simd_vec_report.txt
 */


void element(float *z, const float *x, const float *y, size_t n) {
...
int main(int argc, char **argv){
    ...
    float *z, *x, *y;
    if (posix_memalign((void**)&z, 64, n * sizeof(float)) != 0 ||
        posix_memalign((void**)&x, 64, n * sizeof(float)) != 0 ||
